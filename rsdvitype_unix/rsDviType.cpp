...
bool do_page()
{
	...
	{ 
		...
		{ 
			...
			else {sprintf(temp_buf, "setchar%d", p);_major(temp_buf);}
			goto fin_set;
		}
			//@<Translate a |set_char| command@>
		else switch(o) {
			...
			case four_cases(put1):
				...
				break;
			case set_rule:
				_major("setrule"); goto fin_rule;
				break;
			case put_rule: 
				_major("putrule"); goto fin_rule;
				break;
			//@<Cases for commands |nop|, |bop|, \dots, |pop|@>@;

			case nop: 
				_minor("nop"); goto done;
				break;
			case bop: 
